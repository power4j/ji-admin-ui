    ...
    <d2-container :class="{'page-compact':crud.pageOptions.compact}">
        ...
        >
          ...
          <div slot="header">
            ...
            <el-button v-permission="'sys:role:add'" size="mini" type="primary" @click="addRow"><i class="el-icon-plus"/>新增</el-button>
...
export default {
  ...
  methods: {
    ...
    pageRequest (query) {
      return roleApi.getPage(query)
    },
    addRequest (row) {
      return roleApi.addObj(row)
    },
    updateRequest (row) {
      return roleApi.updateObj(row)
    },
    delRequest (row) {
      return roleApi.delObj(row.id)
    },
    ...
    updateChecked (id) {
      return roleApi.getPermission(id).then(ret => {
        let checkedIds = ret.data ? ret.data.map(o => o.resourceId) : []
        // 找出所有的叶子节点
    ...
    updatePermission (roleId) {
      ...
      roleApi.permit(roleId, this.menuIds).then(() => {
